...
$\textbf{Answer:}$ Since $X_n$ is uniformly bounded and $X_n\to X$ uniformly, then $X_n\to X$ pointwise, also a.e., and $X$ is bounded, because for any specific $n_0$, we have 
$$
\begin{aligned}
\sup_{\omega\in\Omega}|X(\omega)|&=\sup_{\omega\in\Omega}|X(\omega)-X_{n_0}(\omega)+X_{n_0}(\omega)|\\
&\leq\sup_{\omega\in\Omega}|X(\omega)-X_{n_0}(\omega)|+\sup_{\omega\in\Omega}|X_{n_0}(\omega)|,\\
\end{aligned}
$$
since $X_n\to X$ uniformly, the first part of RHS is finite, and the second part is also bounded because $X_n$ is uniformly bounded, so we can let $Y(\omega)\equiv M=\sup_{\omega\in\Omega}|X(\omega)-X_{n_0}(\omega)|+\sup_{\omega\in\Omega}|X_{n_0}(\omega)|$, then $Y\in L_1$ on $(\Omega,\mathcal{B},P)$, by using the DCT, we have $E(X_n)\to E(X)$.

$\textbf{Note:}$ 
1. We should mention that $X_n\to X$ almost everywhere,
2. We only need some specific $n_0$ to build the inequality.
